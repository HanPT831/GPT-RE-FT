...
from prepro import SciercProcessor
from evaluation import compute_f1
from model import REModel
from torch.cuda.amp import GradScaler
import wandb

def save_trained_model(output_dir, model, processor):
    ...
    processor.tokenizer.add_tokens(processor.new_tokens)
    processor.tokenizer.save_pretrained(output_dir)


def train(args, model, train_features, benchmarks, processor):
    ...
    save_trained_model(args.output_dir, model, processor)
    for tag, features in benchmarks:
        f1, output = evaluate(args, model, features, tag=tag)
        wandb.log(output, step=num_steps)
...
def evaluate(args, model, features, tag='dev'):
    ...
    _, _, max_f1 = compute_f1(keys, preds)
...
def main():
    ...
    parser.add_argument("--data_dir", default="./data/scierc", type=str)
    parser.add_argument("--model_name_or_path", default="roberta-large", type=str)
    parser.add_argument("--output_dir", default="./models", type=str, required=True,
    ...
    parser.add_argument("--run_name", type=str, default="scierc")
    ...
    processor = SciercProcessor(args, tokenizer)
    ...
    train(args, model, train_features, benchmarks, processor)
